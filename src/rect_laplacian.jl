...
# Function to evaluate the analytical energies.
function analytical_energies(B::RectBilliard, k::Int)
    energies_ana = []
    for nx in 1:k
        for ny in 1:k
            E_ana = pi^2 * ( (nx/B.Lx)^2 + (ny/B.Ly)^2 ) # Analytical formula for energies of the rectangular billiard
