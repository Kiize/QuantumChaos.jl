# Evaluate the total area of the stadium as the sum of their parts. 
#Assuming left and right circles are the same, we just need to sum the area of the rectangle to the total area of a circle of radius = S.circ_left.radius.
...
function plot_stadium_eigenstate(k, R::RectGeom, N::Int, f::Figure, f2::Figure, flattened_states, coeff, ik::Int, S::BunStadium)
    ...
    # Save figures.
    save("figs/figs_stadium/rect_eigenstate_k$(k)_ik$(ik).png", f)
    save("figs/figs_stadium/rect_eigenstate_k$(k)_ik$(ik)_cont.png", f2)
    #f, f2
    display(f)
    #display(f2)
end